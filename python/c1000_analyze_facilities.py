...
def consolidate_address():
    ...
    try:
        df_consolidated = df_provided.append(retrieve_df('address_consolidated'))
...
def lookup_address(df):
    ...
    try:
        ...
        addresses_provided = retrieve_df('source_fda')
    ...
    zips = list(df['Zip'])
    countries = list(df['Country/Area'])
    ...
    for i in range(len(zips)):
        ...
        address = str(names[i])  + ' , '
        address = address + str(zips[i]) + ' , '
        address = address + str(cities[i]) + ' , '
        address = address + str(states[i]) + ' , '
        address = address + str(countries[i])
